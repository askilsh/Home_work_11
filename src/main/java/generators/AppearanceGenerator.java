...
public class AppearanceGenerator implements Generator<Appearance> {
    ...
    public final Appearance buildResponse() {
        Hair hair;
        HairBuilder builder = new HairBuilder();
        if (hairLength > 0) {
            hair = (hairLength > 4) ? builder.setTypeLong().setColor(hairColor).build()
                    : builder.setTypeShort().setColor(hairColor).build();

        } else {
            hair = builder.setNoType().build();
        }
